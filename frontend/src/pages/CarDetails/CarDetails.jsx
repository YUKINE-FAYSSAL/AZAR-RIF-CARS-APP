...
const CarDetails = () => {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      await api.post(`/car/${car._id}/comment`, {
        username: newComment.name,
        comment: newComment.message,
        rating: rating,
      });
      
      const updated = await api.get(`/car/${id}`);
  ...
  const handleReservation = async () => {
    ...
    try {
      await api.post(`/reserve/${car._id}`, {
  ...
  const handleReply = async (commentId) => {
    ...
    if (reply) {
      try {
        await api.put(`/comment/${commentId}/reply`, { reply });
        const updated = await api.get(`/car/${id}`);
        setCar(updated.data.car);
        setComments(updated.data.comments || []);
...
<img
  src={`http://localhost:5000/api/car/${car._id}/images/${image}`}
