...
const handleSubmit = (e) => {
  ...
  for (let i = 0; i < files.length; i++) {
    data.append('contrats', files[i]);
  }

  api.post('/admin/contrats', data, {
    headers: {
  ...
  const handleDelete = (filename) => {
    ...
    api.delete(`/admin/contrats/${filename}`)
      .then(() => fetchContrats())
    ...
    <div className="contrats-admin">
      ...
      <div className="contrats-actions">
        <form onSubmit={handleSubmit} className="upload-form">
          ...
          <button type="submit" className="btn-upload" disabled={!files.length}>
            Téléverser
          </button>
        </form>
          ...
          {filtered.map(c => (
            <div key={c.filename} className="contrat-card">
              ...
              <div className="contrat-actions">
                <a
                  href={`/api/admin/contrats/download/${c.filename}`}
                  target="_blank"
