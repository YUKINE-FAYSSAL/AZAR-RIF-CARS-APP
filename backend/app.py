...
def user_profile():
    if request.method == 'GET':
        ...
        if file and allowed_file(file.filename):
            ...
            file.save(filepath)
            update_data["profile_img"] = f"/static/uploads/{filename}"

        users_collection.update_one(
            {"email": current_user.email},
            {"$set": update_data}
        )
        user = users_collection.find_one({"email": update_data.get("email", current_user.email)}, {"password": 0})
...
def upload_contrats():
    if 'contrats' not in request.files:
        return jsonify({'error': 'Aucun fichier'}), 400

    files = request.files.getlist('contrats')
    print("Fichiers reçus:", files)

    # ✅ On utilise le même dossier que GET/DELETE
    for file in files:
        filename = secure_filename(file.filename)
        full_path = os.path.join(CONTRATS_DIR, filename)
        print("Chemin de sauvegarde:", full_path)
        file.save(full_path)

    return jsonify({'message': 'Contrats uploadés'}), 200
...
def verify_admin_password():
    ...
    # Load user from DB to get real password hash
    user = users_collection.find_one({"email": current_user.email})

    if not user or 'password' not in user:
        return jsonify({"valid": False, "error": "User not found or password missing"}), 401
...
def get_stats():
    try:
        ...
        return jsonify({
            "totalUsers": total_users or 122,
            "totalReservations": total_reservations or 234,
            "totalCars": total_cars or 10
        }), 200
    except Exception as e:
